...
PREDICT_RESULT=data/dataset_shared/final_result_predict_xlm-roberta_jaen_3epochs.jsonl
OUTFILE=data/dataset_shared/final_result_predict_xlm-roberta_jaen_3epochs.tsv

python src/prepare_review.py < <(cat data/dataset_shared_initial/training-all.jsonl data/dataset_shared/final_result.jsonl) > data/preprocessing_table/all-feats-review.tsv
python src/prepare_product.py < <(cat data/dataset_shared_initial/training-all.jsonl data/dataset_shared/final_result.jsonl) > data/preprocessing_table/all-feats-product.tsv
python src/prepare_customer.py < <(cat data/dataset_shared_initial/training-all.jsonl data/dataset_shared/final_result.jsonl) > data/preprocessing_table/all-feats-customer.tsv
